...
class Visual:
    ...
    def __init__(self,max_x,max_y):
        ...
        self.canvas = self.tk.Canvas(self.root, width=self.max_y*self.zoom*5, height=self.max_x*self.zoom)      #create window
        self.canvas.pack()
...
class Simulation:
    ...
    def __init__(self,
        ...
        self.max_x = int(max_gen * self.cw_speed + self.cw_size) #*2 for margin -> variance
        self.max_y=max_y
        if self.asexual:
            self.sexratio=1
    ...
    def run(self,repeats,startpop,max_gen):             
        for rep in xrange(repeats):
            ...
            while (max_gen>self.time and len(self.individuals)>0):            
                ...
                self.cw_loc=init_cw_loc + np.random.normal(self.cw_speed*self.time, self.cw_speed_variance+0.000000000001)
                ...
                if (self.visualisation) and ((self.time == t1) or (self.time == t2) or (self.time == t3) or (self.time == t4)):
            ...
            if self.visualisation:            
                ...
                self.visual.save('anchestor%s.eps'%(str(rep)))
            #self.visual.reset()
            del self.survivors[:]
            del self.individuals[:]
...
if __name__ == '__main__':                
    simulation=Simulation(max_gen=200,
                          max_y=50,
                          cw_size=80,
                          cw_speed=2,
                          cw_speed_variance=10,
                          maxtime=1000,
                          startpop=20000,
