    ...
    def get_male_gene(self,x,y):
        if self.male_genes[x,y]!=None:
            if len(self.male_genes[x,y])==1:
                ...
                nr_of_males=len(self.male_genes[x,y])
                random_index=np.random.randint(nr_of_males)
                winning_male_gene=self.male_genes[x,y][random_index]
...
class Generalist():
    def __init__(self,x ,y,max_y ,kernelshape,environment,density,anchestor_info,sexratio):
            ...
            #withincw=self.environment.cw_check(self.x)
                ...
                if suitable>=1:
                    ...
                    if self.male_sex:
                        ...
                        #self.density.add_male_gene(self.x,self.y,[self.kernelshape,anchestor_info]) 
                        self.density.add_male_gene(self.x,self.y,self.kernelshape) 
                ...
                for r in xrange(n_seeds):   #For all seeds check if they mutate on one of the two genes and transfer the parent traits into the new individual (+ the mutation)
                    ...
                    if np.random.random()<mutationrate:
                        ...
                        if new_gene+mutation<0:
                            mutation=-mutation
                    anchestor_info=self.anchestor_info[:]
                    offspring.append(Generalist(self.x, self.y,self.max_y,new_gene+mutation,self.environment,self.density,anchestor_info,self.sexratio))        
        ...
        for c in xrange(self.startpop):
            while True:
                ...
                if self.environment.suitability_check(x,y):    #blijven herhalen tot plant in goed habitat gedropt wordt
                    break
            kernelshape=np.random.uniform(0,2)
            anchestor_info=[]
            self.generalists.append(Generalist(x,y,self.max_y,kernelshape,self.environment,self.densloc,anchestor_info,self.sexratio)) 
