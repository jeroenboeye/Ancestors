...
class Visual:
    ...
    def __init__(self,max_x,max_y):
        ...
        self.max_y=max_y
        self.root = tk.Tk()
        self.canvas = tk.Canvas(self.root, width=self.max_y*self.zoom*5, height=self.max_x*self.zoom)      #create window
        self.canvas.pack()
    ...
    def create (self, x, y, gene):
        ...
        self.drawing=self.canvas.create_rectangle(zy-factor+self.max_y*self.zoom*4, zx-factor, zy+factor+self.max_y*self.zoom*4, zx+factor, outline=str(Hex),fill=str(Hex))
        return self.drawing
    ...
    def draw_line(self,pos1,pos2,gene):        
        ...
        self.canvas.create_line(pos1[1]*self.zoom+self.max_y*self.zoom*4,
                                (self.max_x-pos1[0])*self.zoom,
                                pos2[1]*self.zoom+self.max_y*self.zoom*4,
...
class Simulation:
    ...
    def __init__(self,
        ...
        self.popsize = []
        self.max_x = max_gen * self.cw_speed + self.cw_size*2 #*2 for margin -> variance
        self.max_y=max_y
        if self.asexual:
            self.sexratio=1
            self.equilibrium_density=equilibrium_density/2.0
            title='asexual_y_%s_cw_%s_dens_%s_r_%s_maxgen_%s.txt'%(str(self.max_y),str(self.cw_size),str(self.equilibrium_density),str(self.growthrate),str(max_gen))
        else:
            self.sexratio=sexratio
            self.equilibrium_density=equilibrium_density
            title='sexual_y_%s_cw_%s_dens_%s_r_%s_maxgen_%s.txt'%(str(self.max_y),str(self.cw_size),str(self.equilibrium_density),str(self.growthrate),str(max_gen))
    ...
    def run(self,repeats,startpop,max_gen):             
        for rep in xrange(repeats):
            ...
            while (max_gen>self.time and len(self.individuals)>0):            
                ...
                if (self.visualisation):
                    self.popsize.append(len(self.individuals))
                    n_linneages.append(len(lin1))
                    plt.clf()
                    plt.xlabel('Time')
                    plt.ylim([0,9000])
                    plt.plot(self.popsize,'r')[0]
                    plt.plot(n_linneages,'b')[0]
                    plt.draw()
                    if ((self.time == t1) 
                    or (self.time == t2) 
                    or (self.time == t3) 
                    or (self.time == t4)):
                        self.draw_snapshot(t1,t2,t3,t4)
                for ind in self.individuals:
                    self.move(ind)
                if self.visualisation:            
                    self.visual.update()
            ...
            if self.visualisation:            
                ...
                plt.plot(n_anchestors,'g')[0]
                plt.draw()
            #self.visual.reset()
            del self.survivors[:]
            del self.individuals[:]
        
    def draw_snapshot(self,t1,t2,t3,t4):
        if self.time == t1:
            t = 0
        elif self.time == t2:
            t = 1
...
if __name__ == '__main__':                
    simulation=Simulation(max_gen=140,
                          max_y=50,
                          cw_size=40,
                          cw_speed=1,
                          cw_speed_variance=5,
