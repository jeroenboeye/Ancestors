...
class Weergave:
    ...
    def __init__(self,max_x,max_y):
        ...
        self.max_y=max_y
        self.root = tk.Tk()
        self.canvas = tk.Canvas(self.root, width=self.max_x*self.zoom, height=self.max_y*self.zoom*3)      #create window
        self.canvas.pack()
    ...
    def draw_line(self,pos1,pos2,genes,extremes,lower):
        if lower:
            gene = genes[0]
        else:
            gene = genes[1]
        ...
        if lower:
            self.canvas.create_line(pos1[0]*self.zoom,
                                    pos1[1]*self.zoom+self.max_y*self.zoom*2,
                                    pos2[0]*self.zoom,
                                    pos2[1]*self.zoom+self.max_y*self.zoom*2,
    ...
    def get_male_gene(self,x,y):
        if self.male_genes[x,y]!=None:
            if len(self.male_genes[x,y])==1:
                ...
                self.male_genes[x,y] = [winning_male_gene]
...
class Simulation:
        ...
        for c in xrange(self.startpop):
            ...
            while True:
                ...
                if self.environment.suitability_check(x,y):    #blijven herhalen tot plant in goed habitat gedropt wordt
                    break
            kernelshape1=np.random.uniform(0,2)
            kernelshape2=np.random.uniform(0,2)
    ...
    def move(self,ind):
        distance=self.densloc.kernelsample(sum(ind.kernelshape)/2.0)
...
if __name__ == '__main__':                
    simulation=Simulation(max_x=64,
                          max_y=32,
                          maxtime=1000,
                          startpop=10000,
